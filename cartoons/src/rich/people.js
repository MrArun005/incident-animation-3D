...
export function person(g, x, y, L, pose = {}, dir = 1, s = 1, t = 0) {
  ...
  const ank = L.long ? [ll[2], rl[2]] : [k1, k2], hemDrop = L.long ? -10 : (L.kid ? 12 : 14);
  const hemY = L.long ? Math.max(ank[0][1], ank[1][1]) + hemDrop : Math.max(k1[1], k2[1]) * 0.5 + hip[1] * 0.5 + hemDrop;
  const hemB = [Math.min(ank[0][0], ank[1][0]) - L.limb * (L.long ? 1.4 : 0.9), hemY];
  const hemF = [Math.max(ank[0][0], ank[1][0]) + L.limb * (L.long ? 1.4 : 0.9), hemY];
  ...
  if (L.shawl) {
    ...
    blob(g, L.shawl, 3);
    g.save(); g.clip();
    if (L.shawlPlain) { g.strokeStyle = rgba(OL, 0.25); g.lineWidth = 2; for (let i = 0; i < 3; i++) { g.beginPath(); g.moveTo(-tw + 4 + i * 10, -L.torso + 2); g.lineTo(bw - 20 + i * 8, -6); g.stroke(); } }
  ...
  if (L.collar) { g.beginPath(); g.moveTo(-tw * 0.6, -L.torso + 2); g.quadraticCurveTo(4, -L.torso + 26, tw * 0.9, -L.torso + 4); g.quadraticCurveTo(4, -L.torso + 14, -tw * 0.6, -L.torso + 2); blob(g, L.collar, 2); for (let i = 0; i < 3; i++) { g.fillStyle = ['#c0302a', '#2a8a5a', '#c0302a'][i]; g.beginPath(); g.arc(-2 + i * 7, -L.torso + 14 + (i === 1 ? 3 : 0), 2.6, 0, TAU); g.fill(); } }
...
function head(g, hc, rot, L, P, t) {
  ...
  if (L.bun && !L.bald && !L.crown && !L.turban) { g.beginPath(); g.arc(-R * 0.85, -R * 0.2, R * 0.42, 0, TAU); blob(g, L.hair, 2.5); }
  if (L.crown && L.hairLong) { g.beginPath(); g.moveTo(-R * 0.9, -R * 0.1); g.quadraticCurveTo(-R * 1.5, R * 0.9, -R * 1.1, R * 1.6); g.lineTo(-R * 0.5, R * 0.9); g.closePath(); blob(g, L.hair, 2.5); }
  // Neck stub.
  ...
  // Hair cap.
  if (L.bald) { g.fillStyle = rgba('#000000', 0.08); g.beginPath(); g.arc(-R * 0.1, -R * 0.2, R * 0.8, Math.PI, TAU); g.fill(); }
  else { g.beginPath(); g.moveTo(-R * 1.03, R * 0.25); g.arc(0, -R * 0.02, R * 1.05, Math.PI * 0.95, Math.PI * 1.9);
  ...
  if (L.earrings) { g.strokeStyle = L.earrings; g.lineWidth = 3; g.beginPath(); g.arc(-R * 0.3, R * 0.45, R * 0.13, 0, TAU); g.stroke(); }
  if (L.turban) {
    g.beginPath(); g.moveTo(-R * 1.08, R * 0.05); g.quadraticCurveTo(-R * 1.25, -R * 1.25, 0, -R * 1.35); g.quadraticCurveTo(R * 1.15, -R * 1.25, R * 1.02, -R * 0.12); g.quadraticCurveTo(0, -R * 0.38, -R * 1.08, R * 0.05); blob(g, L.turban, 3);
  ...
  if (L.crown) {
    ...
    const cq = g.createLinearGradient(-R, 0, R, 0); cq.addColorStop(0, '#b8862a'); cq.addColorStop(0.5, '#f2cf5a'); cq.addColorStop(1, '#a8761f');
    blob(g, cq, 3);
    g.strokeStyle = '#8a5a14'; g.lineWidth = 2.5; for (const yy of [-0.75, -1.2, -1.6]) { g.beginPath(); g.moveTo(-R * 0.85, R * yy); g.quadraticCurveTo(0, R * (yy - 0.2), R * 0.85, R * yy); g.stroke(); }
    for (const [jx, jy, col] of [[0.05, -0.95, '#c0302a'], [-0.45, -0.9, '#2a8a5a'], [0.5, -0.9, '#2a8a5a'], [0.05, -1.45, '#3a6ab0']]) { g.beginPath(); g.arc(R * jx, R * jy, R * 0.11, 0, TAU); blob(g, col, 1.5); }
    g.beginPath(); g.arc(0, -R * 2.2, R * 0.13, 0, TAU); blob(g, '#f2cf5a', 1.5);
  }
